...
- 当前代码只看到既有 `codex_doctor` 入口（frontend `src/services/tauri.ts` -> backend `src-tauri/src/codex/mod.rs`），尚未看到 proposal 里新增的 preview contract 落地。
- repo 内未检索到 `codex_preview_launch_profile`、Launch Configuration editor、workspace inherit 可视化等核心符号；说明 Phase 1 仍停留在提案阶段，尚未进入实现闭环。
- 现有 `codexBin` / `codexArgs` 能力底座依然存在，但还没有被收口成用户可理解的 `Launch Configuration` 产品体验。
- 因此本提案继续保持“待实施”状态；下一步应先补 backend effective preview / doctor 对齐，再进入 settings UI 与 workspace override 可见性实现。
